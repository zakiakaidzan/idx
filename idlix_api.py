...
def get_m3u8_by_id(video_id: str, content_type: str):
    ...
    title = unquote(
        bs.find("meta", {"itemprop": "name"}).get("content")
    )
    poster = bs.find(
        "img", {"itemprop": "image"}
    ).get("src")

    # 2️⃣ AJAX embed (type dari parameter: movie / tv)
    ajax = session.post(
    ...
    if ajax.status_code != 200 or "embed_url" not in ajax.json():
    ...
    if player.status_code != 200 or "videoSource" not in player.json():
